...
```java
<b>오버라이딩</b> : 같은 메소드 이름, 같은 인자 목록으로 상위 클래스의 메소드를 재정의
<b>오버로딩</b> : 같은 메소드 이름, 다른 인자 목록으로 다수의 메소드를 중복 정의
```
...
```java
<b>public</b> : 모두가 접근 가능
<b>protected - 상속</b> : 패키지내의 클래스에서 접근 가능
<b>default</b> : 같은 패키지 내의 클래스에서 접근 가능
<b>private</b> : 본인만 접근 가능
```
